...
## Поправки к дорожной карте (поправка от 15.09.2026 по результатам аудита стадии 1)

По результатам аудита Стадии 1 (CHECK-0.1 – CHECK-0.7, анализ фактических логов Фаз 1–3 и аудит разбиения ExDark) ревьюером утверждены три поправки к дорожной карте:

- **П1. RUN-5.1 и RUN-5.2 отменяются.** Базовая ветвь мозаичной серии не нужна.
  *Причина:* Завершённые прогоны E4' не показали зависимости качества от доли эпох без мозаики. Три точки при 100 эпохах на одной архитектуре и одном сиде: 0 % чистых эпох — 0,6903 / 0,4350; 10 % — 0,6937 / 0,4348; 50 % — 0,6929 / 0,4323. Размах 0,34 п.п. при межзапусковом отклонении 0,26 п.п., то есть 1,3 сигмы. Гипотеза о вреде мозаичной аугментации на тёмных данных не подтвердилась, достраивать схему до 2x3 ради несуществующего эффекта не нужно. Экономия 4 GPU-часа.
- **П2. Серия E5' добавляется как обязательная.**
  *Причина:* Установлено, что она не запускалась: каталогов `e5p__*` на сервере нет. Четыре прогона (базовая модель и конфигурация после SPPF на 40 и 60 эпохах, сид 0, close_mosaic=10) остаются в плане и становятся единственным кандидатом на объяснение зависимости качества от длительности обучения (Блок 5).
- **П3. В блок 2 добавляется оценка по отложенной тестовой выборке.**
  *Причина:* Установлено, что тестовая часть ExDark (2563 изображения) на сервере не размещалась и ни разу не использовалась — ни в обучении, ни в отборе чекпойнтов, ни в калибровке. Это чистая отложенная выборка. Разместить её, прогнать `model.val()` по всем сохранённым `best.pt` и получить проверку обобщения без единого нового обучающего прогона. Ноль GPU-часов на обучение. Материал для статьи C3.
...
| 2 | Восстановление диагностики Фазы 2 + Тест (П3) | 0 | 1–2 часа |
| 3 | Дозапуски для статьи C1 | 4 | 1 день |
| 4 | Дозапуски для статьи C2 | 6–8 | 1,5 дня |
| 5 | Завершение и достройка Фазы 3 (с учётом П1, П2) | 8 | 2 дня |
...
if not hasattr(self, "_shape_dbg"):
    ...
    print("[SHAPE] iou", tuple(iou.shape),
          "| nwd", tuple(nwd.shape),
          "| alpha", tuple(alpha.shape) if torch.is_tensor(alpha) else type(alpha).__name__,
...
for f in sorted(glob.glob("artifacts/phase2/diag/rho_*.csv")):
    ...
    rows.append({
        "run": tag, "lambda_tv": lam,
        "pearson": round(d["pearson"], 4), "spearman": round(d["spearman"], 4),
        "L_mean": round(d["L_mean"], 4), "L_std": round(d["L_std"], 4),
...
## CALC-2.5. Оценка обобщающей способности по отложенной тестовой выборке (поправка от 15.09.2026 по результатам аудита стадии 1, П3)

Тестовая часть ExDark (2563 изображения) на сервере не размещалась и никогда не участвовала ни в обучении, ни в отборе чекпойнтов, ни в калибровке гиперпараметров. Это чистая отложенная выборка. Разместить её и выполнить `model.val()` по всем сохранённым `best.pt`:

```bash
# Разместить test в datasets/ExDark/images/test и datasets/ExDark/labels/test
python scripts/eval_test_split.py \
  --data exdark.yaml \
  --split test \
  --ckpts runs/detect/runs/*/weights/best.pt \
  --out tables/C3_test_generalization.csv
```

Ноль GPU-часов на обучение, строгая проверка обобщения для статьи C3.
...
p = argparse.ArgumentParser(); p.add_argument("--summary"); p.add_argument("--out")
a = p.parse_args()
d = pd.read_csv(a.summary); d.columns = [c.strip() for c in d.columns]
...
for col in ["mAP50", "mAP50_95"]:
    ...
    se = np.sqrt(vx/len(x) + vy/len(y)); tc = stats.t.ppf(0.975, dfw)
    diff = y.mean() - x.mean()
    sp = np.sqrt(((len(x)-1)*vx + (len(y)-1)*vy) / (len(x)+len(y)-2))
    rows.append({"metric": col, "n_base": len(x), "n_rfd": len(y),
                 "base_mean": round(x.mean(), 4), "base_sd": round(x.std(ddof=1), 4),
                 "rfd_mean": round(y.mean(), 4), "rfd_sd": round(y.std(ddof=1), 4),
                 "delta_pp": round(100*diff, 3),
                 "welch_t": round(float(t), 3), "welch_p": round(float(pv), 4), "df": round(dfw, 2),
                 "ci95_lo_pp": round(100*(diff-tc*se), 3), "ci95_hi_pp": round(100*(diff+tc*se), 3),
                 "mw_U": float(u), "mw_p": round(float(pu), 4),
...
## Статус прогонов Фазы 3 и поправки

По результатам аудита Стадии 1 установлено:
1. Серия E4' (мозаичная аугментация) завершена (прогоны `close0` и `close50`). Метрики mAP50 составили: 0 % чистых эпох — 0,6903 / 0,4350; 10 % — 0,6937 / 0,4348; 50 % — 0,6929 / 0,4323. Размах 0,34 п.п. при межзапусковом отклонении 0,26 п.п. (1,3 сигмы). Зависимость от доли чистых эпох отсутствует.
2. Прогоны **RUN-5.1 и RUN-5.2 отменяются** (поправка от 15.09.2026 по результатам аудита стадии 1, П1). Базовая ветвь не требуется, гипотеза о вреде мозаики не подтвердилась. Экономия: 4 GPU-часа.
3. Серия **E5' добавляется как обязательная** (поправка от 15.09.2026 по результатам аудита стадии 1, П2). На сервере серия E5' ранее не запускалась.

## Обязательная серия E5'. Длительность обучения и эффективность выборки

Четыре прогона для статьи C3:
```bash
python scripts/train_phase3.py --run e5p__baseline-ep40__s0 --data exdark.yaml --device 0
python scripts/train_phase3.py --run e5p__baseline-ep60__s0 --data exdark.yaml --device 0
python scripts/train_phase3.py --run e5p__postsppf-ep40__s0 --data exdark.yaml --device 0
python scripts/train_phase3.py --run e5p__postsppf-ep60__s0 --data exdark.yaml --device 0
```
Вместе с точками на 100 эпохах (RUN-3.1 для базы и Фазой 1 для postsppf на сиде 0) формируется кривая длительности обучения (40, 60, 100 эпох) на едином сиде 0.
...
need = ["tables/C1_table1.csv", "tables/C1_stats.csv", "tables/C2_table1.csv",
        "tables/C2_table2.csv", "tables/C2_diag.csv", "tables/C3_table1.csv",
        "tables/C3_table2.csv", "tables/C3_regime.csv",
        "artifacts/phase0/bench_gpu_rtx3070ti.csv", "artifacts/phase0/env.txt"]
...
tar czf artifacts_v2.tar.gz tables/ figs/ artifacts/ \
  $(for d in runs/*/; do echo "$d/results.csv" "$d/args.yaml"; \
...
| 7 | `e5p__baseline-ep40__s0` | база, 40 эпох, close 10 (П2) | 5 | C3 | 0.8 |
| 8 | `e5p__baseline-ep60__s0` | база, 60 эпох, close 10 (П2) | 5 | C3 | 1.2 |
| 9 | `e5p__postsppf-ep40__s0` | postsppf, 40 эпох, close 10 (П2) | 5 | C3 | 0.8 |
| 10 | `e5p__postsppf-ep60__s0` | postsppf, 60 эпох, close 10 (П2) | 5 | C3 | 1.2 |
| 11 | `e3p__capctrl__s1` | контроль ёмкости, сид 1 | 5 | C1, C3 | 2 |
| 12 | `e3p__capctrl__s2` | контроль ёмкости, сид 2 | 5 | C1, C3 | 2 |
| — | `e4p__baseline-close0__s0` | *Отменён (поправка П1)* | 5 | — | — |
| — | `e4p__baseline-close50__s0` | *Отменён (поправка П1)* | 5 | — | — |

Прогон 6 подтверждён дефектом A5 (FIX-1.4). Итого около 18–20 GPU-часов.
